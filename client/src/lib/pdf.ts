...
export const generateInvoicePDF = async (transaction: Transaction) => {
  ...
  yPos += 20;
  doc.setFontSize(10);
  doc.text('Thank you for choosing Catchy Momos!', 20, yPos);
  doc.text('For support, contact us at support@catchymomos.com', 20, yPos + 7);
  
  doc.save(`catchymomos-invoice-${transaction.id.slice(-6)}.pdf`);
};
...
export const generateMenuSalesPDF = async (menuSales: Array<{name: string, count: number}>, selectedDate?: Date) => {
  ...
  doc.setFontSize(10);
  doc.text('Generated by Catchymomos Analytics', 20, yPos);
  ...
  const filename = `Catchymomos-menu-sales-${fileDate}.pdf`;
  doc.save(filename);
};
